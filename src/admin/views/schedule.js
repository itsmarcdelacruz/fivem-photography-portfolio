...
function renderWeek(c, shoots, weekStart) {
  ...
  const dayHTML = days
    .map((d, i) => {
      ...
      const dots = dayShots
        .slice(0, 3)
        .map((s) => '<span class="week-dot dot-' + escHtml(s.status) + '"></span>')
        .join('');
      return (
        '<div class="week-day' +
        (dayShots.length ? ' has-shoot' : '') +
        '">' +
  ...
  const stripHTML =
    ...
    escHtml(fmtDate(weekStart) + ' – ' + fmtDate(end)) +
    '</span>' +
...
function renderBoard(c, shoots) {
  ...
  const boardHTML = ['booked', 'shooting', 'delivered']
    .map((status) => {
...
function shootCardHTML(s) {
  ...
  return (
    '<div class="shoot-card shoot-card-' +
    escHtml(s.status) +
    '" data-id="' +
    escHtml(String(s.id)) +
    '">' +
    ...
    '<span class="shoot-source-tag">' +
    escHtml(sourceTag) +
    '</span>' +
    (isDelivered
      ? '<button class="shoot-archive-btn">Mark Paid → Archive</button>'
      : advLabel
        ? '<button class="shoot-advance-btn">' + escHtml(advLabel) + '</button>'
        : '') +
    '</div>' +
    '</div>' +
...
function openAddModal(c, shoots) {
  ...
  const typeChipsHTML = SHOOT_TYPES.map(
    (t) => '<button class="type-chip" data-type="' + escHtml(t) + '">' + escHtml(t) + '</button>'
  ).join('');
