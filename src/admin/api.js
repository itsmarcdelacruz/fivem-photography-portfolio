...
export const api = {
  photos: {
    ...
    remove: (id, force = false) =>
      authJson('/api/photos/' + id + (force ? '?force=1' : ''), 'DELETE')
  },
  collections: {
    ...
    replacePhotos: (id, photos) =>
      authJson('/api/admin/collections/' + id + '/photos', 'PUT', photos),
    reorder: (ids) => authJson('/api/admin/collections/order', 'PATCH', { ids })
  },
  commissions: {
    list: () => authJson('/api/commissions', 'GET'),
    updateStatus: (id, s) => authJson('/api/commissions/' + id, 'PATCH', { status: s }),
    promote: (id) => authJson('/api/commissions/' + id + '/promote', 'POST'),
    archive: (id) => authJson('/api/commissions/' + id + '/archive', 'POST'),
    remove: (id) => authJson('/api/commissions/' + id, 'DELETE')
  },
  settings: { get: () => req('/api/settings'), update: (b) => authJson('/api/settings', 'PUT', b) },
  shoots: {
    list: () => authJson('/api/shoots', 'GET'),
    create: (b) => authJson('/api/shoots', 'POST', b),
    update: (id, b) => authJson('/api/shoots/' + id, 'PATCH', b),
    archive: (id) => authJson('/api/shoots/' + id + '/archive', 'POST')
  }
};
