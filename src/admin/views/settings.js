...
export async function initSettings(c) {
  ...
  lbl.textContent = 'Commission availability';
  const hint = document.createElement('div');
  hint.className = 'settings-hint';
  hint.textContent = 'Controls the badge on the public site';
  ...
  check.type = 'checkbox';
  check.id = 'availCheck';
  if (s.availability === 'open') check.checked = true;
  ...
  lblBadge.className = 'settings-label';
  lblBadge.htmlFor = 'availLabel';
  lblBadge.textContent = 'Badge text';
  const input = document.createElement('input');
  input.type = 'text';
  input.id = 'availLabel';
  input.className = 'settings-input';
  input.value = s.availability_label || '';
  input.placeholder = 'Open for July';
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      await api.settings.update({
        availability: check.checked ? 'open' : 'closed',
        availability_label: input.value.trim()
      });
