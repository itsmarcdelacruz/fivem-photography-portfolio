...
function renderPhotos(c, photos, collections) {
  ...
  const queue = createUploadQueue({
    uploadOne: async (file, context, onProgress) => {
      ...
      const photo = { ...created, ...uploaded, id: created.id, thumb_url: uploaded.thumbUrl, title, category: context.category };
  ...
  queue.subscribe(items => {
    queueElement.replaceChildren(...items.map(item => {
      ...
      if (item.status === "failed" || item.status === "queued") {
        ...
        button.addEventListener("click", () => item.status === "failed"
  ...
  c.querySelector("#photoInput").addEventListener("change", event => addFiles(Array.from(event.target.files)));
  ...
  c.querySelector("[data-bulk-hide]").addEventListener("click", () => batch({ is_published: false }));
  c.querySelector("[data-bulk-show]").addEventListener("click", () => batch({ is_published: true }));
