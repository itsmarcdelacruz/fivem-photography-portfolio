...
export async function initOverview(c) {
  ...
  let commissions = [];
  try {
    commissions = (await api.commissions.list()).commissions;
  } catch {
    /* non-fatal: render overview without commissions */
  }
  ...
  const activity = [
    ...
    ...commissions.map((c2) => ({
      type: 'commission',
      label: c2.name + ' (' + (c2.shoot_type || 'request') + ')',
      date: c2.created_at || ''
    }))
  ]
  ...
  c.innerHTML =
    ...
    (settings.availability === 'open' ? ' checked' : '') +
    '><span>' +
    escHtml(settings.availability_label) +
    '</span></label>' +
      ...
      ? activity
          .map(
            (a) =>
              ...
              escHtml(String(a.date).slice(0, 10)) +
              '</span>' +
              '</li>'
          )
          .join('')
  ...
  document.getElementById('availCheck').addEventListener('change', function () {
    ...
    const check = this;
    api.settings.update({ availability: isOpen ? 'open' : 'closed' }).catch((err) => {
